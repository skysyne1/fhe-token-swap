...
import { decryptValue, publicDecrypt, batchDecryptValues } from "../core";
...
export function useDecrypt() {
  ...
  const decrypt = useCallback(
    async (handle: string, contractAddress: string, signer: any) => {
  ...
  const decryptMultiple = useCallback(
    async (handles: string[], contractAddress: string, signer: any) => {
      setIsDecrypting(true);
      setError("");

      try {
        const result = await batchDecryptValues(
          handles,
          contractAddress,
          signer
        );
        ...
        setError(
          err instanceof Error ? err.message : "Multiple decryption failed"
        );
        throw err;
      } finally {
        setIsDecrypting(false);
